...
<section>
    ...
    <p class="body-text">
        How do we find the coefficients that minimize the loss function? There
        are two main approaches for logistic regression: gradient descent and
        maximum likelihood estimation. We’ll briefly discuss both here.
    </p>
    <h2 class="body-secondary-header">Gradient Descent</h2>
    <p class="body-text">
        A common way to estimate coefficients is to use gradient descent. In
        gradient descent, the goal is to minimize the Log-Loss cost function
        over all samples. This method involves selecting initial parameter
        values, and then updating them incrementally by moving them in the
        direction that decreases the loss. At each iteration, the parameter
        value is updated by the step size (otherwise known as the learning rate)
        and the gradient. The gradient is the direction and rate of the fast
    ...
    <p class="body-text">
        ...
        observing the data by using Maximum Likelihood Estimation (MLE). In MLE,
        the goal is to maximize the likelihood of the data. It turns out,

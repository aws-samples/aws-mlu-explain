 <script>
    ...
    $: updateCounts = function(xPos) {
        ...
        // if ($stage === 'none') {
        //     console.log('in none stage')
        //     circs.attr('opacity', 1);
        // }else if ($stage === 'precision') {
        //     console.log('in precision stage')
        //     // lighten everything to left of DB
        //     pos.filter(function(d) { return $xScale.invert(select(this).attr('cx')) >= xPos }).attr('opacity', 1)
        //     neg.filter(function(d) { return $xScale.invert(select(this).attr('cx')) >= xPos }).attr('opacity', 1)
        //     // show everything to right of db
        //     neg.filter(function(d) { return $xScale.invert(select(this).attr('cx')) < xPos }).attr('opacity', 0.5)
        //     pos.filter(function(d) { return $xScale.invert(select(this).attr('cx')) < xPos }).attr('opacity', 0.5)

        // } else {
        //     console.log('in recall stage')
        //     // recall: reduce all negative opacity
        //     neg.filter(function(d) { return $xScale.invert(select(this).attr('cx')) >= xPos }).attr('opacity', .6)
        //     neg.filter(function(d) { return $xScale.invert(select(this).attr('cx')) < xPos }).attr('opacity', .6)
        // }
    }
    ...
    function dragged(event, d) {
        ...
        if (xPos <= 0.01 || xPos >= 14.6) {
            ...
            select(this).raise().attr("x", event.x);
            // recalculate metrics
            updateCounts(xPos)
        }
    }
